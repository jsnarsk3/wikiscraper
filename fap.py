...
def get_movie_attr(): 
    ...
    if title != None:
        if first:
            ret = [movie for movie in ret.items() if movie[1]['title'] == title]
            first = False
        else:
            ret = [movie for movie in ret if movie[1]['title'] == title]
    if grossing != None:
        if first:
...
def get_movie(movie_title):
    ret = [movie for movie in data_movie.items() if movie[1]['title'] == movie_title]
    return jsonify(dict(ret)),200
...
def put_movie(movie_title):
    '''
    This function updates a movie with attributes changed
    :param movie_title: passed in url
    '''
    movie = [movie for movie in data_movie.items() if movie[1]['title'] == movie_title][0]
    my_dict = request.get_json()
    for key in my_dict:
        if key == 'title':
            movie[1]['title'] = my_dict['title']
        if key == 'json_class':
...
def delete_movie(movie_title):
    ...
    :param movie_title:
    '''
    if movie_title not in data_movie.keys(): return abort(404)
    deleted = data_movie[movie_title]
    ret = [item for item in data_movie.items() if item[1]['title'] != movie_title]
    data[1] = dict(ret)
    with open("test_data_write.json", "w+") as my_file:
        json_string = json.dumps(data, default=parser.dumper, indent=4, sort_keys=True, separators=(',', ':'))
        my_file.write(json_string)
